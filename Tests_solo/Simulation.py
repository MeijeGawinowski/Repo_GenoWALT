...
class Simulation():
    ...
    def __init__(self):
        self.selfing = 0.95 # proportion of autogamy in the population
    ...
    def create_FitnessSimulation(self,parpop):
        ...
        parpop.evolve(initOps=self.initOps, preOps=self.preOps, matingScheme=self.mating, finalOps=self.finalOps,gen=1)
    ...
    def create_controlledSimulation(self,parpop):
        ...
        if self.attr == "drawing" :
            for i in range(len(Des)):
                p = Des[i]/DesTot
                prop.append(p)
            par = list(np.random.choice(list(range(N_ind)),size=newSize, replace=True, p=prop))
